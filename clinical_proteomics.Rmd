...
dataset = analysis_quickstart(
  dataset,
  filter_min_detect = 2,         # each peptide must have a good confidence score in at least N samples per group
  ...
  dea_algorithm = c("ebayes", "deqms", "msempire", "my_dea_stats"), #, "msempire", "msqrob" # statistics; apply multiple methods in parallel/independently
  dea_qvalue_threshold = 0.1,                      # threshold for significance of adjusted p-values in figures and output tables
  dea_log2foldchange_threshold = 0.4,                # threshold for significance of log2 foldchanges. 0 = disable, NA = automatically infer through bootstrapping
...
my_dea_stats = function(peptides, samples, eset_peptides, eset_proteins, input_intensities_are_log2, random_variables, ...) {
  ...
  pval = rep(NA, nrow(x))
  for(i in 1:nrow(x)) {
    # if(length(x[i,cols_grp1]) != length(x[i,cols_grp2]))
    # {
    # bigger_by = abs(length(x[i,cols_grp1]) - length(x[i,cols_grp2]))
    # if((length(x[i,cols_grp1]) - bigger_by) == length(x[i,cols_grp2]))
    # {
    #   x1 <- sample(x[i,cols_grp1], size = (length(x[i,cols_grp1]) - bigger_by))
    # }else{
    #   x1 <- x[i,cols_grp1]
    # }
    # if((length(x[i,cols_grp2]) - bigger_by) == length(x[i,cols_grp1]))
    # {
    #   x2 <- sample(x[i,cols_grp2], size = (length(x[i,cols_grp2]) - bigger_by))
    # }else{
    #   x2 <- x[i,cols_grp2]
    # }
    # }
    
    i_tt = t.test(x[i,cols_grp1], x[i,cols_grp2], alternative = "two.sided", paired = F, var.equal = F)
    pval[i] = i_tt$p.value
  }
  # log2 fold-change
  log2FC = rowMeans(x[,cols_grp2,drop=F], na.rm=T) - rowMeans(x[,cols_grp1,drop=F], na.rm=T)

  ### 3) create a result tibble that contains all columns required for downstream compatibility with this pipeline; protein_id, pvalue, qvalue, foldchange.log2, algo_de
  ...
  cat("Example custom DEA implementation, my_dea_stats(), yielded", sum(is.finite(result$qvalue) & result$qvalue<=0.1), "hits at qvalue<=0.1\n")
  return(result)
}
...
dataset = analysis_quickstart(dataset,
                              ...
                              dea_qvalue_threshold = 0.5,
                              dea_log2foldchange_threshold = NA, # estimate a fold-change threshold for proteins to be significant
                              output_qc_report = T, # disabled in this document, but do create a QC report when testing code to review those volcano's, p-value and foldchange distributions !
